...
def _check_environment():
    ...
    if frontend_env.exists():
        ...
        try:
            ...
            if "VITE_SUPABASE_ANON_KEY" in frontend_env_content:
                ...
                issues.append(
                    "VITE_SUPABASE_ANON_KEY missing in frontend .env"
                )
        except Exception as e:
    ...
    if openrouter_key:
        ...
        warnings.append(
            "OPENROUTER_API_KEY missing (needed for OpenRouter models)"
        )
...
def _run_frontend(
    ...
    if not frontend_dir.exists():
        ...
        print(
            "Make sure you're running this from the AgentBeats project root."
        )
        print(
    ...
    try:
        if mode == "dev":
            print(
                f"Development server will be available at http://{host}:{port}"
            )
            ...
            print(
                f"Building and previewing production build for {webapp_version}..."
            )
            # First build
            subprocess.run(
                "npm run build",
...
def _install_frontend(webapp_version: str):
    ...
    try:
        ...
        print(
            f"Frontend dependencies installed successfully for {webapp_version}!"
        )
        sys.exit(0)
    except subprocess.CalledProcessError as e:
...
def _run_backend(
    host: str, backend_port: int, mcp_port: int, reload: bool, dev_login: bool
):
    ...
    def start_backend():
        ...
        try:
            ...
            print(
                f"Starting AgentBeats Backend on http://{host}:{backend_port}"
            )
            uvicorn.run(
                "backend.app:app", host=host, port=backend_port, reload=reload
            )
        except Exception as e:
...
def main():
    ...
    load_scenario_parser.add_argument(
        "--register_agents",
        action="store_true",
        help="Register agents to backend",
    )
    load_scenario_parser.add_argument(
        "--backend",
        help="Backend URL (optional) when registering agents",
    ...
    backend_parser.add_argument(
        "--host", default="localhost", help="Backend host"
    )
    backend_parser.add_argument(
        "--backend_port",
        type=int,
    ...
    backend_parser.add_argument(
        "--dev_login",
        action="store_true",
        help="Enable development mode - skip authentication for all API calls",
    )
    ...
    frontend_parser.add_argument(
        "--backend_url", required=True, help="Backend URL for API proxy"
    )
    frontend_parser.add_argument(
        "--dev_login",
        action="store_true",
        help="Enable development mode - show dev login button on login page",
    )
    ...
    deploy_parser = sub_parser.add_parser(
        "deploy",
        help="Deploy complete AgentBeats stack (backend + frontend + MCP)",
    )
    ...
    deploy_parser.add_argument(
        ...
        type=int,
        default=9001,
        help="MCP server port (default: 9001)",
    )
    deploy_parser.add_argument(
        "--dev_login", action="store_true", help="Enable development mode"
    )
    ...
    if args.cmd == "run_agent":
        ...
        manager = ScenarioManager(
            scenario_root=pathlib.Path(args.scenario_root)
        )
        manager.load_scenario(mode=args.launch_mode)
        time.sleep(
            10
        )  # TODO: should check aliveness here; can implement later
        manager.start_battle(
            backend_url=args.backend, frontend_url=args.frontend
        )
