...
def _deploy_current_terminal(
    ...
    try:
        ...
        print(
            f"Starting Frontend in {deploy_mode} mode on port {frontend_port}..."
        )
        if deploy_mode == "dev":
            frontend_cmd = [
                sys.executable,
        ...
        if deploy_mode == "dev":
            ...
            print(
                "[Info] Services started. Use 'pm2 list' to check frontend status."
            )
            print(
                "[Info] Use 'pm2 logs agentbeats-ssr' to check frontend logs."
            )
...
def _deploy_separate_terminals(
    deploy_mode: str,
    backend_port: int,
    frontend_port: int,
    mcp_port: int,
    ...
    if deploy_mode == "dev":
        ...
        try:
            subprocess.run(
                "pm2 --version", check=True, capture_output=True, shell=True
            )
    ...
    services = [("Backend", backend_cmd), ("Frontend", frontend_cmd)]
    ...
    for name, cmd in services:
        ...
        if system == "Windows":
            ...
            if not terminal_opened:
                print(
                    f"Warning: Could not open terminal for {name}. Please run manually: {cmd}"
                )
    ...
    if deploy_mode == "build":
        print(f"[Frontend] http://localhost:3000")
        print(
            f"[Frontend] Running with PM2 (use 'pm2 logs agentbeats-ssr' for logs)"
        )
    else:
        print(f"[Frontend] http://localhost:{frontend_port}")
    print(f"[MCP]      http://localhost:{mcp_port}")
    ...
    try:
        ...
        if deploy_mode == "dev":
            ...
            try:
                ...
                print(
                    f"[Production] npx pm2 detected, will use PM2 for frontend in tmux..."
                )
                ...
                print(
                    "[Warning] PM2 not found in tmux mode, using preview mode..."
                )
                frontend_cmd = f"{sys.executable} -m agentbeats run_frontend --frontend_mode preview --host localhost --frontend_port {frontend_port} --backend_url http://localhost:{backend_port}"
        ...
        subprocess.run(
            [
                ...
                "-t",
                f"{session_name}:0.0",
                f"echo 'Starting Backend...' && {backend_cmd}",
                "Enter",
            ],
            check=True,
        )
        ...
        subprocess.run(
            [
                ...
                "-t",
                f"{session_name}:0.2",
                f"echo 'Starting Frontend...' && {frontend_cmd}",
                "Enter",
            ],
            check=True,
        )
        ...
        if deploy_mode == "build":
            ...
            print(
                f"[Frontend] Running with PM2 (use 'npx pm2 logs agentbeats-ssr' for logs)"
            )
        else:
            print(
                f"[Frontend] http://localhost:{frontend_port} (Bottom right pane)"
            )
        print("=" * 50)
        print(
            f"To attach to tmux session: tmux attach-session -t {session_name}"
        )
        print(f"To kill tmux session: tmux kill-session -t {session_name}")
        if deploy_mode == "build":
            print(
                "Frontend is managed by PM2. Use 'npx pm2 list' to check status."
            )
