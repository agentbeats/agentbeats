...
class ScenarioService:
    ...
    def __init__(self, config: Dict[str, Any], scenario_dir: Path):
        ...
        if self.type == "docker_compose":
            ...
            raise ValueError(
                f"Unknown service type: {self.type} for service {self.name}"
            )
    ...
    def _start_docker_compose(self):
        ...
        if self.process.returncode != 0:
            raise RuntimeError(
                f"Failed to start {self.name}: {stderr.decode()}"
            )
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            with urllib.request.urlopen(
                self.health_check, timeout=5
            ) as response:  # TODO: haven't tested this
...
class ScenarioAgent:
    ...
    def __init__(self, config: Dict[str, Any], scenario_dir: Path):
        ...
        if "launcher_host" not in config:
            raise ValueError(
                f"launcher_host is required for agent {self.name}"
            )
        if "launcher_port" not in config:
            raise ValueError(
                f"launcher_port is required for agent {self.name}"
            )
        ...
        self.participant_requirements = config.get(
        ...
        if self.is_green and self.participant_requirements:
            for req in self.participant_requirements:
                if not isinstance(req, dict):
                    raise ValueError(
                        f"participant_requirements must be a list of dict for green agent {self.name}"
                    )
                required_fields = [
                    "role",
                    "name",
                ...
                for field in required_fields:
                    if field not in req:
                        raise ValueError(
                            f"participant_requirements item missing {field} for green agent {self.name}"
                        )
                if req["role"] not in ["blue_agent", "red_agent"]:
                    raise ValueError(
                        f"role must be 'blue_agent' or 'red_agent' for green agent {self.name}"
                    )
                if not isinstance(req["required"], bool):
                    raise ValueError(
                        f"required must be boolean for green agent {self.name}"
                    )
...
class ScenarioManager:
    ...
    def load_scenario(self, mode: str = None):
        ...
        scenario_name = self.config["scenario"][
            "name"
        ]  # Must provide a name in scenario.toml
        launch_config = self.config.get(
            "launch", {}
                    ...
                    if service.health_check:
                        ...
                        for i in range(max_retries):
                            if service.is_healthy():
                                ...
                                break
                            print(
                                f"Waiting for {service.name} to be healthy... ({i+1}/{max_retries})"
                            )
                            time.sleep(2)
                        else:
                            print(
                                f"⚠️  {service.name} health check failed, continuing anyway..."
                            )
    ...
    def _start_agents_tmux(self, config: Dict[str, Any]):
        ...
        if not shutil.which("tmux"):
            print(
                "❌ tmux is not installed. Falling back to separate terminals."
            )
            self._start_agents_terminals()
            return

        launch_config = self.config.get("launch", {})
        session_name = launch_config.get(
            "tmux_session_name",
            f"agentbeats-{self.config['scenario']['name']}",
        )
        ...
        subprocess.run(
            ["tmux", "select-layout", "-t", session_name, "tiled"], check=True
        )
    ...
    def _start_agents_background(
        ...
        for agent in self.agents:
            ...
            thread = threading.Thread(
                target=handle_output, args=(agent.name, process)
            )
            thread.daemon = True
            thread.start()
    ...
    def register_agent_to_backend(
        self, agent: ScenarioAgent, backend_url: str = "http://localhost:9000"
    ) -> Optional[str]:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                print(
                    f"Registering agent {agent.name} (attempt {attempt}/{max_retries})..."
                )
                ...
                response = requests.post(
                    f"{backend_url}/agents", json=register_data, timeout=30
                )
                ...
                if response.status_code == 201:
                    ...
                    print(
                        f"✅ Registered agent {agent.name} with ID: {agent_id}"
                    )
                    return agent_id
                else:
                    print(
                        f"⚠️ Failed to register agent {agent.name} (attempt {attempt}): {response.status_code} {response.text}"
                    )
                ...
                print(
                    f"⚠️ Error registering agent {agent.name} (attempt {attempt}): {str(e)}"
                )
                if attempt < max_retries:
                    print(f"Waiting {retry_delay} seconds before retry...")
                    time.sleep(retry_delay)
        ...
        print(
            f"❌ Failed to register agent {agent.name} after {max_retries} attempts"
        )
        return None
    ...
    def create_battle(
        ...
        try:
            ...
            response = requests.post(
                f"{backend_url}/battles", json=battle_data, timeout=30
            )
            ...
            if response.status_code == 201:
                ...
                print(
                    f"❌ Failed to create battle: {response.status_code} {response.text}"
                )
                return None
    ...
    def start_battle(
        self, backend_url: str, frontend_url: str
    ) -> Optional[str]:
        """Load scenario, register agents, create battle, and return frontend URL"""
        print(
            f"Starting battle for scenario: {self.config['scenario']['name']}"
        )
        ...
        green_agent_id = self.register_agent_to_backend(
            green_agent, backend_url
        )
