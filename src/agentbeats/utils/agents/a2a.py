...
async def get_agent_card(target_url: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        resolver = A2ACardResolver(
            httpx_client=httpx_client, base_url=target_url
        )
...
async def send_message_to_agent(
    target_url: str, message: str, timeout: Optional[float] = None
) -> str:
    ...
    try:
        ...
        # log the message
        print(f"Sending message to {target_url}: {message}")
...
async def send_message_to_agents(
    target_urls: List[str], message: str, timeout: Optional[float] = None
) -> Dict[str, str]:
        ...
        try:
            if timeout is not None:
                response = await asyncio.wait_for(
                    send_message_to_agent(url, message), timeout=timeout
                )
            else:
...
async def send_messages_to_agents(
    ...
    if len(target_urls) != len(messages):
        raise ValueError(
            f"Number of URLs ({len(target_urls)}) must match number of messages ({len(messages)})"
        )
    ...
    tasks = [
        send_to_single_agent(url, message)
        for url, message in zip(target_urls, messages)
    ]
