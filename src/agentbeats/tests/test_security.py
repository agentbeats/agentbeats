...
def test_file_upload_security():
        ...
        with open(test_file, "rb") as f:
            ...
            for malicious_name in malicious_filenames:
                ...
                response = requests.post(
                    f"{base_url}/assets/uploads/battle/{battle_id}",
                    files=files,
                    data=data,
                )
                ...
                if response.status_code == 400:
                    ...
                    print(
                        f"✗ Failed to block malicious filename: {malicious_name} (Status: {response.status_code})"
                    )
                    return False
    finally:
    ...
    try:
        with open(large_file.name, "rb") as f:
            ...
            if (
                ...
                print(
                    f"✗ Failed to block oversized file (Status: {response.status_code})"
                )
                return False
    finally:
    ...
    for ext in disallowed_extensions:
        ...
        try:
            with open(test_file, "rb") as f:
                ...
                if (
                    response.status_code == 400
                    and "not allowed" in response.text.lower()
                ):
                    print(f"✓ Blocked disallowed extension: {ext}")
                else:
                    print(
                        f"✗ Failed to block disallowed extension: {ext} (Status: {response.status_code})"
                    )
                    return False
        finally:
            if os.path.exists(test_file):
        ...
        try:
            with open(test_file, "rb") as f:
                ...
                if response.status_code == 400 and (
                    ...
                    or "mime type" in response.text.lower()
                ):
                    print(f"✓ Blocked disallowed MIME type: {mime_type}")
                else:
                    print(
                        f"✗ Failed to block disallowed MIME type: {mime_type} (Status: {response.status_code})"
                    )
    ...
    try:
        with open(test_file, "rb") as f:
            ...
            if (
                response.status_code == 400
                and "empty" in response.text.lower()
            ):
                print("✓ Blocked empty file")
            else:
                print(
                    f"✗ Failed to block empty file (Status: {response.status_code})"
                )
                return False
    finally:
        if os.path.exists(test_file):
    ...
    try:
        with open(test_file, "rb") as f:
            ...
            data = {"user_id": ""}
            response = requests.post(
                f"{base_url}/assets/uploads/avatar/user",
                files=files,
                data=data,
            )
            if (
                ...
                print(
                    f"✗ Failed to block empty user ID (Status: {response.status_code})"
                )
                return False
            ...
            data = {"agent_id": ""}
            response = requests.post(
                f"{base_url}/assets/uploads/avatar/agent",
                files=files,
                data=data,
            )
            if (
                ...
                print(
                    f"✗ Failed to block empty agent ID (Status: {response.status_code})"
                )
                return False
    finally:
        if os.path.exists(test_file):
    ...
    try:
        with open(test_file, "rb") as f:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✗ Valid file upload failed (Status: {response.status_code})"
                )
                return False
    finally:
        if os.path.exists(test_file):
...
def test_static_asset_security():
    ...
    for malicious_path in malicious_paths:
        ...
        if response.status_code == 400:
            ...
            print(
                f"✗ Failed to block path traversal: {malicious_path} (Status: {response.status_code})"
            )
            return False
    ...
    try:
        response = requests.get(
            f"{base_url}/assets/static/test/test_security.txt"
        )
        if response.status_code == 200:
            print("✓ Valid static asset access succeeded")
        else:
            print(
                f"✗ Valid static asset access failed (Status: {response.status_code})"
            )
            return False
    finally:
...
def main():
    ...
    try:
        ...
        print(
            f"File Upload Security: {'✅ PASSED' if upload_security else '❌ FAILED'}"
        )
        print(
            f"Static Asset Security: {'✅ PASSED' if static_security else '❌ FAILED'}"
        )
        ...
        print(
            "✗ Could not connect to server. Make sure the backend is running on localhost:9000"
        )
        return False
