...
@router.get("/services/agent_card/{agent_id}", tags=["Services"], status_code=status.HTTP_200_OK)
...
@router.put("/services/agent_card/{agent_id}", tags=["Services"], status_code=status.HTTP_200_OK)
...
@router.get("/services/launcher_status", tags=["Services"], status_code=status.HTTP_200_OK)
async def services_check_launcher_status(
    launcher_url: str = Query(..., description="Launcher base URL to check, e.g. https://host:port")
):
