...
def load_config():
    ...
    config_path = os.path.join(
        os.path.dirname(__file__), "..", "backend_cfg.toml"
    )
    try:
...
async def wait_agents_ready(
    agent_ids: list, timeout: int = 300, poll_interval: float = 5
):
    """
...
# --- Helper functions for process_battle refactor ---
def lock_agents(agent_ids):
    """Lock all agents by setting their status to 'locked'."""
    for agent_id in agent_ids:
        agent = db.read("agents", agent_id)
        if agent:
            agent["status"] = "locked"
...
def reset_agent(agent, agent_id, extra_args=None):
    """Reset an agent using its launcher URL."""
    launcher_url = agent["register_info"].get("launcher_url")
    if not launcher_url:
        return False
    return asyncio.run(
        a2a_client.reset_agent_trigger(
            launcher_url, agent_id=agent_id, extra_args=extra_args or {}
        )
    )


def handle_battle_error(
    battle_id,
    error_msg,
    log_msg=None,
    log_detail=None,
    ...
    """Handle battle error: update state, log, stats, unlock agents, broadcast."""
    battle = db.read("battles", battle_id)
    if battle:
        battle["state"] = "error"
        battle["error"] = error_msg
        db.update("battles", battle_id, battle)
        if log_msg:
            add_system_log(battle_id, log_msg, log_detail)
        if update_stats:
...
async def reset_opponents_and_collect_info(battle, opponent_ids):
    """Reset all opponent agents and collect info for green agent."""
    opponent_info_send_to_green = [
        {"name": op["name"]} for op in battle["opponents"]
    ]
    for idx, op_id in enumerate(opponent_ids):
        ...
        if not op_reset:
            return idx, op_id, op
        opponent_info_send_to_green[idx]["agent_url"] = op[
            "register_info"
        ].get("agent_url")
    return None, None, opponent_info_send_to_green


# --- Refactored process_battle ---
async def process_battle(battle_id: str):
    try:
        ...
        if not green_agent:
            handle_battle_error(
                battle_id, "Green agent not found", "Green agent not found"
            )
            return
        ...
        for opponent_info in battle["opponents"]:
            ...
            if not opponent:
                handle_battle_error(
                    battle_id,
                    f"Opponent agent {opponent_id} not found",
                    f"Opponent agent {opponent_id} not found",
                )
                return
        ...
        # Lock the agents
        lock_agents([battle["green_agent_id"]] + opponent_ids)
        add_system_log(battle_id, "Agents locked")

        # Reset the green agent
        ...
        if not green_reset:
            handle_battle_error(
                battle_id,
                "Failed to reset green agent",
                "Green agent reset failed",
            )
            return

        # Reset opponent agents and collect info for green agent
        idx, op_id, opponent_info_send_to_green = (
            await reset_opponents_and_collect_info(battle, opponent_ids)
        )
        if idx is not None:
            op_name = battle["opponents"][idx].get("name")
            handle_battle_error(
                battle_id,
                f"Failed to reset {op_name}: {op_id}",
                f"{op_name} reset failed",
                {"opponent_id": op_id, "opponent_name": op_name},
            )
            return
        ...
        battle_timeout = green_agent["register_info"].get(
            "battle_timeout",
            CONFIG.get("battle", {}).get("default_battle_timeout", 300),
        )
        ...
        notify_success = await a2a_client.notify_green_agent(
            green_agent_url, opponent_info_send_to_green, battle_id
        )
        ...
        handle_battle_error(battle_id, str(e))
...
def create_battle(battle_request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        battle_record = {
            ...
            "opponents": battle_request[
                "opponents"
            ],  # list of dicts with 'name' and 'agent_id'
