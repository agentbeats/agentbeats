...
class RoleMatcher:
    ...
    async def analyze_agent_for_roles(
        ...
        cache_key = self._get_cache_key(
            green_agent_card, participant_requirements, other_agent_card
        )
        ...
        try:
                    ...
                    if json_match:
                        try:
                            ...
                            if start != -1 and end != -1 and end > start:
                                ...
                                raise ValueError(
                                    f"Could not find JSON structure in response: {content}"
                                )
                    else:
                        raise ValueError(
                            f"Could not extract JSON from response: {content}"
                        )
            ...
            if original_confidence != result["confidence_score"]:
                role_matcher_logger.warning(
                    f"Confidence score clamped from {original_confidence} to {result['confidence_score']}"
                )
            ...
            role_matcher_logger.error(
                f"Error in role analysis for {other_name} vs {green_name}: {str(e)}"
            )
            error_result = {
                "matched_roles": [],
                "reasons": {},
    ...
    def _is_cache_valid(self, cache_key: str) -> bool:
        ...
        if (
            ...
            or cache_key not in self._cache_timestamps
        ):
            return False
