...
class MatchStorage:
    def __init__(self, db_path: str = None):
        if db_path is None:
            ...
            db_dir = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "db", "data"
            )
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Performance indexes
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_matches_green ON agent_matches(green_agent_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_matches_other ON agent_matches(other_agent_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_matches_confidence ON agent_matches(confidence_score DESC)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_roles_match ON match_roles(match_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_roles_name ON match_roles(role_name)"
            )
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                ...
                if "id" not in match_data:
                    ...
                    match_storage_logger.info(
                        f"🆔 Generated new match ID: {match_data['id']}"
                    )
    ...
    def get_matches_for_green_agent(
        self, green_agent_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def get_matches_by_role(
        self, role_name: str, min_confidence: float = 0.0
    ) -> List[Dict[str, Any]]:
    ...
    def delete_match(self, match_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                ...
                conn.execute(
                    "DELETE FROM match_roles WHERE match_id = ?", (match_id,)
                )

                # Delete main match record
                cursor = conn.execute(
                    "DELETE FROM agent_matches WHERE id = ?", (match_id,)
                )
    ...
    def get_match_stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            avg_cursor = conn.execute(
                "SELECT AVG(confidence_score) FROM agent_matches"
            )
            ...
            top_roles = [
                ...
                for row in top_roles_cursor.fetchall()
            ]
