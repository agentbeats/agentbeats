...
def clear_all(match_storage):
    with sqlite3.connect(match_storage.db_path) as conn:
        ...
        conn.commit()
        print(
            f"Cleared {match_count} matches and {role_count} role assignments"
        )
...
def clear_green_agent(match_storage, agent_id):
    with sqlite3.connect(match_storage.db_path) as conn:
        cursor = conn.execute(
            "SELECT COUNT(*) FROM agent_matches WHERE green_agent_id = ?",
            (agent_id,),
        )
        match_count = cursor.fetchone()[0]
        ...
        conn.execute(
            "DELETE FROM match_roles WHERE match_id IN (SELECT id FROM agent_matches WHERE green_agent_id = ?)",
            (agent_id,),
        )
        conn.execute(
            "DELETE FROM agent_matches WHERE green_agent_id = ?", (agent_id,)
        )
        conn.commit()
...
def clear_other_agent(match_storage, agent_id):
    with sqlite3.connect(match_storage.db_path) as conn:
        cursor = conn.execute(
            "SELECT COUNT(*) FROM agent_matches WHERE other_agent_id = ?",
            (agent_id,),
        )
        match_count = cursor.fetchone()[0]
        ...
        conn.execute(
            "DELETE FROM match_roles WHERE match_id IN (SELECT id FROM agent_matches WHERE other_agent_id = ?)",
            (agent_id,),
        )
        conn.execute(
            "DELETE FROM agent_matches WHERE other_agent_id = ?", (agent_id,)
        )
        conn.commit()
        print(f"Cleared {match_count} matches")
