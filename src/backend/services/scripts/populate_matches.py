...
def show_agent_stats(agents):
    ...
    green_agents = len(
        [
            a
            for a in agents
            if a.get("register_info", {}).get("is_green", False)
        ]
    )
...
def get_green_agents(agents):
    return [
        a for a in agents if a.get("register_info", {}).get("is_green", False)
    ]
...
def get_other_agents(agents):
    return [
        a
        for a in agents
        if not a.get("register_info", {}).get("is_green", False)
    ]


def extract_requirements(green_agent):
    requirements = green_agent.get("register_info", {}).get(
...
async def analyze_matches(
    role_matcher, green_agent, other_agents, match_storage
):
    ...
    for other_agent in other_agents:
        ...
        existing_matches = match_storage.get_matches_for_green_agent(
            green_agent_id
        )
        if any(
            m["other_agent_id"] == other_agent_id for m in existing_matches
        ):
        ...
        try:
            ...
            if result.get("matched_roles"):
                ...
                print(
                    f"  {other_agent_id} -> {result['matched_roles']} ({result.get('confidence_score', 0.0):.2f})"
                )
            else:
...
async def populate_all(match_storage, role_matcher, dry_run=False):
    ...
    print(
        f"Found {len(green_agents)} green agents, {len(other_agents)} other agents"
    )
    ...
    print(
        f"{'Would create' if dry_run else 'Created'} {total_matches} matches"
    )
...
async def populate_agent(match_storage, role_matcher, agent_id, dry_run=False):
    ...
    target_agent = next(
        (a for a in agents if a.get("agent_id") == agent_id), None
    )
    if not target_agent:
    ...
    if is_green:
        ...
        matches = await analyze_matches(
            role_matcher, target_agent, other_agents, match_storage
        )
        ...
        print(
            f"{'Would create' if dry_run else 'Created'} {len(matches)} matches"
        )
        ...
        for green_agent in green_agents:
            ...
            try:
                ...
                if result.get("matched_roles"):
                    ...
                    total_matches += 1
                    print(
                        f"  {green_agent_id} -> {result['matched_roles']} ({result.get('confidence_score', 0.0):.2f})"
                    )
...
async def populate_green_agent(
    match_storage, role_matcher, green_agent_id, dry_run=False
):
    agents = get_all_agents()
    show_agent_stats(agents)
    ...
    target_agent = next(
        (
            a
            for a in agents
            if a.get("agent_id") == green_agent_id
            and a.get("register_info", {}).get("is_green", False)
        ),
        None,
    )
    if not target_agent:
...
async def main():
    ...
    group.add_argument(
        "--agent-stats", action="store_true", help="Show agent statistics only"
    )
    ...
    try:
        ...
        if args.stats:
            ...
            await populate_agent(
                match_storage, role_matcher, args.agent, args.dry_run
            )
        elif args.green_agent:
            await populate_green_agent(
                match_storage, role_matcher, args.green_agent, args.dry_run
            )
