    ...
    async def reset_agent_trigger(
        ...
        try:
            ...
            response = await httpx_client.post(
                f"{launcher_url}/reset", json=reset_payload
            )

            if response.status_code != 200:
                logger.error(
                    f"Failed to reset agent at {launcher_url}: {response.status_code}"
                )
                return False
            else:
    ...
    async def notify_green_agent(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"BattleContext objects may not be JSON serializable"
                )
                ...
                try:
                    ...
                    logger.error(
                        "Basic info serializes fine, BattleContext is the issue"
                    )
            ...
            try:
                response = await send_message_to_agent(
                    endpoint, json.dumps(battle_info)
                )
                logger.info(f"Message sent successfully, response: {response}")
            except Exception as send_error:
                logger.error(
                    f"Failed to send message to {endpoint}: {send_error}"
                )
            ...
            logger.info(
                f"Green agent notification {'succeeded' if success else 'failed'}"
            )
            return success

        except Exception as e:
            logger.error(
                f"Error in notify_green_agent for {endpoint}: {str(e)}"
            )
