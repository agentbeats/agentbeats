#!/usr/bin/env python3
"""
Test script to demonstrate role matching with example agent cards and verify database storage.
"""
...
async def test_role_matching():
    """Test role matching with example agent cards."""
    print("🧪 Testing Role Matching with Example Agent Cards")
    print("=" * 60)
    ...
    try:
        deleted_count = match_storage.delete_matches_for_agent("test_green_agent_123")
        print(f"✅ Deleted {deleted_count} test matches from database")
    except Exception as e:
...
def main():
    ...
    try:
        ...
        asyncio.run(test_role_matching())
