<script lang="ts">
  ...
  $: canCancelBattle = battleInProgress && (
    isDevMode || 
    (battle?.created_by && authHook.user?.email && battle.created_by === authHook.user.email)
  );
  
  async function fetchAgentName(agentId: string): Promise<string> {
    try {
      const agent = await agentsHook.getAgentById(agentId);
      return agent?.alias || agent?.agent_card?.name || agentId;
    } catch {
      return agentId;
    }
  }
  ...
  async function fetchGreenAgentInfo(agentId: string): Promise<any> {
    try {
      return await agentsHook.getAgentById(agentId);
    } catch {
      return null;
    }
  }
  ...
  function buildOpponentRoleMap(greenAgentInfo: any, battleOpponents: any[]): Map<string, string> {
    ...
    if (!greenAgentInfo?.participant_requirements || !Array.isArray(battleOpponents)) {
      return roleMap;
    }
    
    const participantReqs = greenAgentInfo.participant_requirements;
  ...
  async function cancelBattle() {
    ...
    try {
      ...
      // Use the battlesHook to update battle
      await battlesHook.updateBattle(battleId, cancelEvent);
  ...
  onMount(async () => {
    ...
    try {
      // Use the battlesHook to get battle by ID
      battle = await battlesHook.getBattleById(battleId);
      ...
      // Connect to WebSocket for real-time updates using the hook
      battlesHook.connectWebSocket();
      
      // Subscribe to battle updates from the hook
      const unsubscribe = battlesHook.subscribe((battlesData) => {
        // Find our specific battle in the updated data
        const updatedBattle = battlesData.battles.find((b: any) => b.battle_id === battleId);
        if (updatedBattle && battle) {
