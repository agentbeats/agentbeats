<script lang="ts">
  ...
  async function loadTopMatches() {
    ...
    try {
      ...
      if (agentType === 'green') {
        ...
        // First get all agents for liveness info using new hooks
        await agents.loadAllAgentsWithLiveness();
        ...
        // Get all agents to find green agents using new hooks
        await agents.loadAllAgentsWithLiveness();
        const greenAgents = agents.greenAgents;
                ...
                opponentMatches.forEach((match: any) => {
                  allMatches.push({
                    ...
                    green_agent: {
                      ...
                      alias: greenAgent.alias || greenAgent.agent_card?.name,
  ...
  function handleAddToCart() {
    ...
    const agentName = agent.alias || agent.agent_card?.name || 'agent';
  ...
  async function handleAddMatchToCart(agentId: string) {
    try {
      // Get the full agent data using hooks
      const agentData = await agents.getAgentById(agentId);
      ...
      const agentName = agentData.alias || agentData.agent_card?.name || 'agent';
