<script lang="ts">
  ...
  onMount(() => {
    const unsubscribeCart = cartStore.subscribe(items => {
      ...
      if (greenAgent?.agent?.participant_requirements) {
        participantRequirements = greenAgent.agent.participant_requirements as Array<{role?: string; [key: string]: any}>;
          ...
          {#if greenAgent}
            <div class="mb-6">
              ...
              <div class="bg-gray-50 border-2 border-gray-200 rounded-lg p-4">
                <AgentChip
                  agent={{
                    identifier: greenAgent.agent.alias || greenAgent.agent.agent_card?.name || 'Unknown Agent',
                    avatar_url: greenAgent.agent?.avatar_url,
                          ...
                          {#if participantSlots[index]}
                            <div class="flex items-center space-x-2">
                              <div class="pointer-events-none">
                                <AgentChip
                                  agent={{
                                    identifier: participantSlots[index]!.agent.alias || participantSlots[index]!.agent.agent_card?.name || 'Unknown Agent',
                                    avatar_url: participantSlots[index]!.agent?.avatar_url,
                  ...
                  {#each opponentAgents as item, index}
                    ...
                    >
                      <div class="flex-1">
                        <AgentChip
                          agent={{
                            identifier: item.agent.alias || item.agent.agent_card?.name || 'Unknown Agent',
