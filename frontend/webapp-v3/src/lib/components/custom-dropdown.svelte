  ...
  >
    <div class="flex-1 text-left">
      {#if selectedOption && selectedOption.agent}
        <AgentChip 
          agent={{
            identifier: selectedOption.agent.alias || selectedOption.agent.agent_card?.name || selectedOption.label,
            avatar_url: selectedOption.agent?.avatar_url,
  ...
  {#if isOpen}
    ...
    >
                    ...
                    <button
                      ...
                      disabled={isDisabled}
                      class="w-full p-2 text-left hover:bg-gray-50 focus:bg-gray-50 focus:outline-none transition-colors duration-150 {isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} {option.value === value ? 'bg-gray-100 border-l-2 border-gray-400' : ''}"
                    >
                      <div class="flex items-center">
                        ...
                        <div class="flex-1">
                          {#if option.agent}
                            <AgentChip
                              agent={{
                                identifier: option.agent.alias || option.agent.agent_card?.name || option.label,
                                avatar_url: option.agent?.avatar_url,
