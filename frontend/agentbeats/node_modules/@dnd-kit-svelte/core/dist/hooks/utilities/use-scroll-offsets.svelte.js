import { defaultCoordinates, getScrollableElement, getScrollCoordinates, getScrollOffsets, } from '../../utilities/index.js';
import { add } from '@dnd-kit-svelte/utilities';
export function useScrollOffsets(elementsFn) {
    const elements = $derived(elementsFn() ?? []);
    let scrollCoordinates = $state(null);
    ...
    const scrollOffsets = $derived.by(() => {
        if (elements.length) {
