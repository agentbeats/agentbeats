...
tmux split-window -v -t $SESSION "cd $PROJECT_DIR && source venv/bin/activate && python scenarios/tensortrust_mock/agent_launcher.py --file scenarios/tensortrust_mock/blue_agent/main.py --port 9010"

# Split Pane 1 vertically for red_agent (Pane 3)
tmux select-pane -t $SESSION:0.1
tmux split-window -v -t $SESSION "cd $PROJECT_DIR && source venv/bin/activate && python scenarios/tensortrust_mock/agent_launcher.py --file scenarios/tensortrust_mock/red_agent/main.py --port 9020"

# Split Pane 2 vertically for green_agent (Pane 4)
tmux select-pane -t $SESSION:0.2
tmux split-window -v -t $SESSION "cd $PROJECT_DIR && source venv/bin/activate && OPENAI_API_KEY=\$OPENAI_API_KEY python scenarios/tensortrust_mock/agent_launcher.py --file scenarios/tensortrust_mock/green_agent/main.py --port 9030 --mcp-url http://localhost:9001/sse"
