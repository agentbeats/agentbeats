...
def process_incoming_message(message: str) -> str:
    ...
    try:
        print(
            f"Processing incoming message: {message[:200]}..."
        )  # Log first 200 chars
        ...
        if (
            "battle_id" in message_data
            or "ssh_host" in message_data
            or "user_persona" in message_data
        ):
...
def handle_challenge_start(challenge_data: str) -> str:
    ...
    try:
        ...
        battle_context = BattleContext(
            battle_id=battle_context_data.get(
                "battle_id", "ctf_password_brute_force"
            ),
            backend_url=battle_context_data.get(
                "backend_url", "http://localhost:9000"
            ),
            agent_name=battle_context_data.get(
                "agent_name", "CTF Red Agent (Competitor)"
            ),
        )
        ...
        print(
            f"Challenge initialized for agent {agent_id} in battle {current_battle_id}"
        )
        return (
            f"Challenge started for {agent_id} in battle {current_battle_id}"
        )
...
async def _try_ssh_password(password: str) -> str:
    ...
    try:
        if not challenge_info:
            print(
                f"DEBUG: challenge_info is None, current_battle_id: {current_battle_id}"
            )
        ...
        if not challenge_info and password:
            print(
                "DEBUG: No challenge info but trying password - challenge message may not have been processed"
            )
        ...
        if ssh_client.connect():
            ...
            find_result = ssh_client.execute(
            ...
            for location in flag_locations:
                ...
                if (
                    flag_result
                    and not flag_result.startswith("Error:")
                    and not "No such file" in flag_result
                ):
                    if flag_result.startswith("Success:"):
                        flag_content = flag_result.replace(
                            "Success:", ""
                        ).strip()
                    else:
                ...
                for script in flag_scripts:
                    ...
                    if (
                        ...
                        if flag_result.startswith("Success:"):
                            ...
                            flag_content = flag_result.strip()
                        print(
                            f"Found flag from script {script}: {flag_content}"
                        )
                        break
            ...
            record_agent_action(
                battle_context,
                "Flag found",
                {"password_used": password, "flag": flag_content},
            )
            ...
            try:
                ...
                print(
                    f"DEBUG: About to submit flag to green agent at {green_agent_url}"
                )
                ...
                # Submit the flag
                submission_result = await submit_flag_to_green_agent(
                    flag_content
                )
...
async def test_password_list(password_list: str) -> str:
    ...
    try:
        ...
        record_agent_action(
            ...
            {"passwords": passwords, "batch_size": len(passwords)},
        )
...
async def submit_flag_to_green_agent(flag_content: str) -> str:
    ...
    try:
        ...
        print(
            f"Submitting flag for {agent_name} in battle {current_battle_id}"
        )
        ...
        response = await send_message_to_agent(
            green_agent_url, json.dumps(flag_submission)
        )
