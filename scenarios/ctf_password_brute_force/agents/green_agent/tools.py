...
async def handle_incoming_message(message: str) -> str:
    ...
    try:
        ...
        return (
            f"Received message of type: {message_data.get('type', 'unknown')}"
        )
...
async def setup_ctf_environment(battle_id: str) -> str:
    ...
    try:
        ...
        if battle_context:
            record_agent_action(
                battle_context,
                "Generated password and persona",
                {
        ...
        possible_arena_paths = [
            ...
            os.path.join(
                os.getcwd(),
                "external/agentbeats_sdk/src/agentbeats/demos/ctf_password_brute_force/arena",
            ),  # From project root
        ]
        ...
        if battle_context:
            record_agent_action(
                battle_context, "Generated flag in container", {"flag": flag}
            )
        ...
        if battle_context:
            record_battle_event(
                ...
                {"container_name": ctf_container_name, "status": "ready"},
            )

            # Log successful setup (blank details)
            record_battle_event(
                battle_context, "CTF environment setup completed", {}
            )
...
def generate_flag_in_container() -> str:
    ...
    try:
        ...
        cmd = (
            f"docker exec {ctf_container_name} python3 /root/generate_flag.py"
        )
        ...
        flag_line = [
            line for line in output_lines if line.startswith("Flag generated:")
        ]
        if flag_line:
...
async def start_competition(battle_id: str, red_agent_urls: list) -> str:
    ...
    try:
        if not flag or not user_persona:
            return (
                "CTF environment not set up. Run setup_ctf_environment first."
            )
        ...
        for url in red_agent_urls:
            ...
            if not red_context:
                print(
                    f"Warning: No BattleContext found for {url}, using default"
                )
                red_context = {
                    "battle_id": battle_id,
            ...
            print(
                f"Prepared challenge message for {red_context.get('agent_name', 'unknown')} at {url}"
            )
        ...
        if battle_context:
            record_battle_event(
                battle_context,
                "Container verified ready, sending challenge to red agents",
                {
        ...
        if battle_context:
            record_agent_action(
                battle_context,
                "Sent challenge message to red agents",
                {"red_agents": red_agents, "send_results": send_results},
            )
...
async def submit_flag(
    battle_id: str, agent_name: str, submitted_flag: str
) -> str:
    ...
    try:
        print(
            f"Validating flag submission - Battle ID: {battle_id}, Agent: {agent_name}"
        )
        ...
        if submitted_flag.strip() == flag:
            ...
            if battle_context:
                record_agent_action(
                    battle_context,
                    f"Winner declared: {agent_name}",
                    {"winner": agent_name, "submitted_flag": submitted_flag},
                )
            ...
            if battle_context:
                record_battle_result(
                    battle_context,
                    f"CTF challenge completed - {agent_name} won",
                    agent_name,
                    {
                        "winner": agent_name,
            ...
            error_msg = (
                f"❌ Incorrect flag submitted by {agent_name}. Keep trying!"
            )
            print(error_msg)
            return error_msg
