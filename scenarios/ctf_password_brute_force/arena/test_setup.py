                ...
                if line.strip():
                    ...
                    if len(parts) == 2:
                        ...
                        for port in required_ports:
                            if (
                                f":{port}->" in ports
                                or f"0.0.0.0:{port}->" in ports
                            ):
                                containers_to_stop.append(container_name)
                                print(
                                    f"⚠️ Found container '{container_name}' using port {port}"
                                )
                                break
    ...
    try:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(
                    f"⚠️ SSH connection failed with sshpass: {result.stderr}"
                )
        ...
        if result.returncode == 0 and "sshd" in result.stdout:
            ...
            print(
                "ℹ️ Manual SSH test: ssh -p 2222 root@localhost (password: password123)"
            )
            return True
        else:
            print(
                f"❌ SSH daemon not running in container. Output: {result.stdout[:200]}..."
            )
            return False
