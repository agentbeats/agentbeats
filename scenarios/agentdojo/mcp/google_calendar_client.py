...
class GoogleCalendarClient:
    ...
    def __init__(
        ...
        initial_events: List[CalendarEvent] = None,
        initial_contacts: List[Contact] = None,
        initial_directory_people: List[DirectoryPerson] = None,
        initial_other_contacts: List[Contact] = None,
        current_day: datetime.date = None,
    ):
        self.user_email = user_email
        self.current_day = current_day or datetime.date.today()
        self.events: Dict[str, CalendarEvent] = {}
        self.contacts: List[Contact] = initial_contacts or []
        self.directory_people: List[DirectoryPerson] = (
            initial_directory_people or []
        )
        ...
        if initial_events:
            ...
            raise ValueError(
                "Initial events must be provided to initialize the client."
            )

    @classmethod
    def init_from_yaml_and_injection(
        cls, yaml_path: str, injection: Dict[str, str]
    ) -> "GoogleCalendarClient":
        """
    ...
    def list_events(
        ...
        if after:
            ...
            events = [
                e
                for e in events
                if self._parse_time(e.start["dateTime"]) >= after_dt
            ]
        ...
        if before:
            ...
            events = [
                e
                for e in events
                if self._parse_time(e.end["dateTime"]) <= before_dt
            ]

        return sorted(events, key=lambda e: e.start["dateTime"])

    def get_event_by_id(
        self, event_id: str, calendar: str = "primary"
    ) -> CalendarEvent:
    ...
    def get_contacts(
        self, page_cursor: Optional[str] = None
    ) -> Dict[str, Any]:
            ...
            "connections": [
                {
                    ...
                    "organizations": [{"name": c.organization}]
                    if c.organization
                    else [],
                }
    ...
    def search_directory_people(
        self, query: str, page_cursor: Optional[str] = None
    ) -> Dict[str, Any]:
        """Search directory people by query."""
        matching_people = [
            p
            for p in self.directory_people
            if query.lower() in p.name.lower()
            or query.lower() in p.email.lower()
            or (p.department and query.lower() in p.department.lower())
            or (p.title and query.lower() in p.title.lower())
        ]
    ...
    def get_availability(
        ...
        for item in items:
            ...
            calendars[calendar_id] = {"busy": busy_times, "errors": []}
...
def _get_client():
    ...
    if _client is None:
        raise ValueError(
            "GoogleCalendarClient is not initialized. Call init_client_from_yaml first."
        )
    return _client
...
def init_client_from_yaml(yaml_path: str, injection: Dict[str, str] = None):
    ...
    if injection is None:
        injection = {}
    _client = GoogleCalendarClient.init_from_yaml_and_injection(
        yaml_path, injection
    )
...
def google_calendar_list_events(
    ...
    try:
        ...
        events = client.list_events(
            calendar=calendar, after=after, before=before
        )
        return {
            "success": True,
            "events": [
...
def google_calendar_get_event_by_id(
    eventId: str, calendar: str = "primary"
) -> Dict[str, Any]:
...
def google_calendar_search_directory_people(
    query: str, paginationParameters: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        client = _get_client()
        result = client.search_directory_people(
            query=query, page_cursor=page_cursor
        )
        return {"success": True, **result}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def google_calendar_get_availability(
    ...
    try:
        ...
        result = client.get_availability(
            time_min=timeMin, time_max=timeMax, time_zone=timeZone, items=items
        )
        return {"success": True, **result}
    except Exception as e:
        return {"success": False, "error": str(e)}
