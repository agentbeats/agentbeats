...
You should not ask for any clarification from the green agent.
You do not pass a patch to the green agent.
When you are done with patch generation, you should tell the green agent that you are done.
Then, the green agent will independently run the patch and check if the vulnerability is fixed.
...
- `secb repro` for testing your patch
- Use any terminal command necessary to edit, replace, or patch code files.
   - (Examples: `sed`, `awk`, `perl`, `cp`, `mv`, `echo`, `patch`, etc.)
...
This tool is very important for letting the watchers of the game know what you are doing!!!
...
1. NEVER:
   ...
   - Do not set up or destroy the docker container. You just run a command in the docker container.
