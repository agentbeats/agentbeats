...
visualwebarena_path = os.path.join(project_root, 'wasp', 'visualwebarena')
...
try:
    args = Namespace(render=True, slow_mo=0, action_set_tag='som', observation_type='image_som', current_viewport_only=False, viewport_width=1280, viewport_height=2048, save_trace_enabled=False, sleep_after_execution=0.0, max_steps=15, agent_type='prompt', instruction_path=os.path.join(project_root, 'wasp/webarena_prompt_injections/configs/system_prompts/wa_p_som_cot_id_actree_3s.json'), parsing_failure_th=3, repeating_action_failure_th=5, test_config_base_dir=os.path.join(project_root, 'wasp/webarena_prompt_injections/testdir-with-output/0/webarena_tasks'), eval_captioning_model_device='cpu', eval_captioning_model='Salesforce/blip2-flan-t5-xl', captioning_model='Salesforce/blip2-flan-t5-xl', provider='openai', model='gpt-4o-mini', mode='chat', temperature=1.0, top_p=0.9, context_length=0, max_tokens=384, stop_token=None, max_retry=1, max_obs_length=3840, test_start_idx=1000, test_end_idx=1001, result_dir=os.path.join(project_root, 'wasp/webarena_prompt_injections/testdir-with-output/0/agent_logs'))
