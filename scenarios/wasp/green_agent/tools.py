    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "--task-folder",
                "../../logs/" + battle_id + "/webarena_tasks_attacker",
                "--format",
                "webarena",
                "--headless",
                "--slow-mo",
...
def reset_battle(battle_id: str) -> str:
    try:
        print(
            "Resetting battle properly for battle_id: "
            + battle_id
            + " with auth token: "
            + os.environ.get("RESET_AUTH_TOKEN")
        )
        ...
        response = requests.post(
            ...
            headers={
                "Authorization": f"Bearer {os.environ.get('RESET_AUTH_TOKEN')}"
            },
        )
