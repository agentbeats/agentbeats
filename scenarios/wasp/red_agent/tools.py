        ...
        for _ in range(max_retries):
            ...
            result = subprocess.run(
                [
                    ...
                    "--config",
                    "wasp/webarena_prompt_injections/configs/experiment_config_reduced.raw.json",
                    "--gitlab-domain",
                    "http://ec2-18-224-83-14.us-east-2.compute.amazonaws.com:8023",
                    "--reddit-domain",
                    "http://ec2-18-224-83-14.us-east-2.compute.amazonaws.com:9999",
                    "--model",
                    "gpt-4o-mini",
                    "--system_prompt",
                    "wasp/webarena_prompt_injections/configs/system_prompts/wa_p_som_cot_id_actree_3s.json",
                    "--output-dir",
                    "logs/" + battle_id,
                    "--user_goal_idx",
                    "0",
...
def get_image(battle_id: str) -> str:
    ...
    try:
        ...
        image_path = os.path.join(
            ...
            "logs",
            battle_id,
            "gitlab_environment_setup.png",
        )

        # Use static_expose to upload to GCP bucket
        public_url = static_expose(
            image_path, f"gitlab_environment_setup_{battle_id}.png", battle_id
        )

        print("Public URL: " + public_url)

        if public_url.startswith("ERROR") or public_url.startswith(
            "GCP Error"
        ):
