...
def handle_battle_start(battle_info_json: str) -> str:
    ...
    try:
        ...
        battle_context = BattleContext(
            battle_id=battle_id, backend_url=backend_url, agent_name=agent_id
        )

        # Log battle start
        record_battle_event(
            battle_context,
            f"Red agent {agent_id} starting battle",
            agent_id,
            {"battle_id": battle_id, "agent_type": "red_agent"},
        )
        ...
        if ssh_credentials:
            ...
            if all([host, port, username, password]):
                client = connect_to_battle_arena(
                ...
                if client:
                    ...
                    log_error(
                        battle_context,
                        "Failed to connect to battle arena",
        ...
        if battle_context:
            log_error(
                battle_context,
                error_msg,
                "battle_start_exception",
                agent_id or "red_agent",
            )
        return error_msg


def connect_to_battle_arena(
...
@ab.tool
def deploy_custom_web_service(
    service_code: str, port: int = 80, filename: str = "custom_service.py"
) -> str:
    ...
    try:
        ...
        execute_ssh_command(
            f"fuser -k {port}/tcp 2>/dev/null || true", ssh_client
        )
        execute_ssh_command(
            f"pkill -f '{filename}' 2>/dev/null || true", ssh_client
        )
        ...
        execute_ssh_command(
            f"nohup python3 /tmp/{filename} > /tmp/{filename}.log 2>&1 &",
            ssh_client,
        )
        ...
        result = execute_ssh_command(
            f"curl -s http://localhost:{port}", ssh_client
        )
        ...
        if result and not result.startswith("curl:"):
            ...
            if battle_context:
                record_agent_action(
                    battle_context,
                    "web_service_deployed",
                    agent_id,
                    {"port": port, "filename": filename, "status": "running"},
                )

            return f"Custom web service deployed and running on port {port}"
        else:
            # Check the log for errors
            log_result = execute_ssh_command(
                f"cat /tmp/{filename}.log", ssh_client
            )
            ...
            if battle_context:
                log_error(
                    battle_context,
                    f"Web service deployment failed on port {port}",
                    "deployment_error",
                    str(agent_id or "red_agent"),
                )
...
def execute_custom_strategy(strategy_commands: str) -> str:
    ...
    try:
        ...
        commands = [
            cmd.strip() for cmd in strategy_commands.split("\n") if cmd.strip()
        ]
        ...
        if battle_context:
            record_agent_action(
                battle_context,
                "custom_strategy_completed",
                str(agent_id or "red_agent"),
                {
                    "commands_executed": len(commands),
                    "successful_commands": len(
                        [r for r in results if "failed" not in r]
                    ),
                },
            )
...
def check_port_status(port: int = 80) -> str:
    ...
    try:
        ...
        port_check = execute_ssh_command(
            f"lsof -i :{port} 2>/dev/null || echo 'Port {port} is free'",
            ssh_client,
        )

        # Try to connect to the port
        curl_result = execute_ssh_command(
            f"curl -s -m 5 http://localhost:{port} 2>/dev/null || echo 'No response from port {port}'",
            ssh_client,
        )
...
def find_available_port(start_port: int = 80, max_port: int = 90) -> str:
    ...
    try:
        ...
        for port in range(start_port, max_port + 1):
            result = execute_ssh_command(
                f"lsof -i :{port} 2>/dev/null || echo 'free'", ssh_client
            )
            if "free" in result:
...
def sabotage_competitors(sabotage_commands: str) -> str:
    ...
    try:
        ...
        # Split commands by newlines and execute each
        commands = [
            cmd.strip() for cmd in sabotage_commands.split("\n") if cmd.strip()
        ]
        ...
        for command in commands:
            try:
                ...
                results.append(
                    f"Sabotage command '{command}' failed: {str(e)}"
                )
