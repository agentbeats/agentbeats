...
def send_battle_info_to_agents(
    battle_id: str,
    agent_urls: List[str],
        ...
        async def send_async():
            ...
            for url in agent_urls:
                ...
                task = asyncio.create_task(
                    send_message_to_agent(url, battle_message)
                )
                tasks.append(task)
...
def start_monitoring_thread(battle_id: str, agent_ids: Dict[str, str]):
    ...
    def monitor_battle_progress():
        ...
        try:
            ...
            if battle_context:
                record_battle_event(
                    battle_context,
                    f"Battle monitoring started for {battle_id}",
                    "green_agent",
                    {
                        "battle_id": battle_id,
            ...
            for i in range(36):
                if not monitoring_active:
                    ...
                    if battle_context:
                        record_battle_event(
                            battle_context,
                            f"Monitoring stopped early at check {i+1}",
                            "green_agent",
                        )
                    break
                ...
                try:
                    ...
                    if result.returncode == 0 and result.stdout.strip():
                        ...
                        if current_serving_agent != "NO_SERVICE":
                            ...
                            print(
                                f"Agent '{current_serving_agent}' is serving (score: {scores[current_serving_agent]})"
                            )
                        else:
            ...
            if battle_context:
                record_battle_event(
                    battle_context,
                    f"Battle monitoring completed for {battle_id}",
                    "green_agent",
                    {
                        "total_checks": 36,
    ...
    monitoring_thread = threading.Thread(
        target=monitor_battle_progress, daemon=True
    )
...
def start_battle_royale(battle_id: str, opponent_infos_json: str) -> str:
    ...
    try:
        ...
        for opponent in opponent_infos:
            ...
            print(
                f"Generated agent ID '{agent_id}' for {opponent['agent_url']}"
            )

        print(
            f"Starting battle royale {battle_id} with {len(opponent_infos)} opponents"
        )
        ...
        record_battle_event(
            battle_context,
            f"Battle royale {battle_id} starting",
            "green_agent",
            {
                "battle_id": battle_id,
        ...
        try:
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run, setup_container(docker_config)
                    )
            ...
            if not docker_result:
                log_error(
                    battle_context,
                    "Failed to setup Docker container",
                    "docker_setup_error",
                    "green_agent",
                )
        ...
        send_battle_info_to_agents(
            battle_id, agent_urls, agent_ids, ssh_credentials
        )
        ...
        record_battle_event(
            battle_context,
            f"Battle royale {battle_id} completed",
            "green_agent",
            {"battle_result": battle_result, "final_scores": scores.copy()},
        )
        ...
        try:
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run,
                        cleanup_container("battle_royale", "arena"),
                    )
            ...
            print(error_msg)
            log_error(
                battle_context, error_msg, "cleanup_error", "green_agent"
            )
...
def get_battle_result(battle_id: str = "", agent_urls: List[str] = []) -> str:
    ...
    try:
        ...
        if not scores:
            print(
                "No agents have scores after full monitoring period - all agents failed to create services"
            )
            ...
            print(
                f"Winner determined: {winner_agent_id} with {winner_score}/{required_checks} checks ({uptime_percentages[winner_agent_id]:.1f}% uptime)"
            )
