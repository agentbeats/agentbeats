...
# Configuration
PROJECT_DIR=$HOME/agentbeats_projects/agentbeats
SETUP_COMMAND="cd $PROJECT_DIR && source venv/bin/activate"
SCENARIO_SETUP_COMMAND="cd scenarios/multi_turn_jailbreaking"
MAIN_MCP_URL="http://localhost:9001/sse"
DOCKER_MCP_URL="http://localhost:9002/sse"
SESSION_NAME="multi_turn_jailbreaking"
...
# Start agents
python scenarios/multi_turn_jailbreaking/start_agents.py
...
# tmux send-keys -t $SESSION_NAME "$(create_banner 'Jailbreaking MCP SERVER')" C-m
# tmux send-keys -t $SESSION_NAME "$SETUP_COMMAND && python -m scenarios.multi_turn_jailbreaking.mcp.mcp_server" C-m
