...
def run_arvo_container(
    ...
    try:
        cmd = [
            "/bin/bash",
            "-c",
            f"timeout -s SIGKILL {cmd_timeout} /bin/arvo 2>&1",
        ]
        container = client.containers.run(
            image=f"n132/arvo:{arvo_id}-{mode}",
            command=cmd,
            volumes={
                str(poc_path.absolute()): {"bind": "/tmp/poc", "mode": "ro"}
            },  # noqa: S108
            detach=True,
        )
        out = container.logs(
        ...
        raise HTTPException(
            status_code=500, detail=f"Running error: {e}"
        ) from None
...
def run_oss_fuzz_container(
    ...
    try:
        ...
        volumes = {
            str(poc_path.absolute()): {"bind": "/testcase", "mode": "ro"}
        }
...
def run_container(
    ...
    **kwargs,
    ...
    elif task_id.startswith("oss-fuzz:") or task_id.startswith(
        "oss-fuzz-latest:"
    ):
...
def submit_poc(
    ...
    if not verify_task(
        payload.task_id, payload.agent_id, payload.checksum, salt=salt
    ):
    ...
    existings = get_poc_by_hash(
        db, payload.agent_id, payload.task_id, poc_hash
    )
    poc_id = uuid4().hex
    if existings:
        if len(existings) > 1:
    ...
    exit_code, docker_output = run_container(
        payload.task_id, poc_bin_file, mode, oss_fuzz_path=oss_fuzz_path
    )
    output_file = poc_dir / f"output.{mode}"
...
def run_poc_id(
    ...
    if len(records) != 1:
        raise HTTPException(
            status_code=500,
            detail=f"{len(records)} PoC records for same poc_id found",
        )
    ...
    if rerun or record.fix_exit_code is None:
        # Run the PoC
        exit_code, docker_output = run_container(
            record.task_id, poc_path, "fix", oss_fuzz_path=oss_fuzz_path
        )
