...
from cybergym.server.server_utils import (
...
def submit_vul(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid metadata format"
        ) from None
...
def verify_all_pocs_for_agent_id(db: SessionDep, query: VerifyPocs):
    ...
    if not records:
        raise HTTPException(
            status_code=404, detail="No records found for this agent_id"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--salt", type=str, default=SALT, help="Salt for checksum"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OSS_FUZZ_PATH,
        help="Path to OSS-Fuzz",
    )
